...
function Test-PineconeApiKey {
    ...
    - Example: pcsk_4WN3H7_Nzw9N2Kedg1TU6uhXkdP4ThXr6mWkm7D2GgVstsiXNgSe9CzfW7B7fZdny4NZLL
    ...
    .EXAMPLE
    Test-PineconeApiKey -ApiKey "pcsk_4WN3H7_Nzw9N2Kedg1TU..."
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($ApiKey)) {
        if (-not $Silent) {
            Write-Host "[ERROR] Pinecone API key is empty" -ForegroundColor Red
        }
        return $false
    }
    ...
    # Check character set (letters, numbers, underscores)
    ...
    # Check for placeholder patterns
    if (Test-PlaceholderPattern -Value $ApiKey -KeyType "Pinecone" -Silent:$Silent) {
        return $false
    }

    if (-not $Silent) {
        Write-Host "[OK] Pinecone API key format validated (75 chars, pcsk_...)" -ForegroundColor Green
    }
    return $true
}
...
function Test-OpenAiApiKey {
    ...
    - Prefix: sk-proj-
    - Length: 164 characters total
    - Characters: Letters (a-z, A-Z), numbers (0-9), hyphens (-), and underscores (_)
    - Example: sk-proj-AbCdEfGh1234567890...
    ...
    .EXAMPLE
    Test-OpenAiApiKey -ApiKey "sk-proj-AbCdEfGh1234567890..."
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($ApiKey)) {
        if (-not $Silent) {
            Write-Host "[ERROR] OpenAI API key is empty" -ForegroundColor Red
        }
        return $false
    }

    # Check prefix
    if (-not $ApiKey.StartsWith("sk-proj-")) {
        if (-not $Silent) {
            Write-Host "[ERROR] OpenAI API key must start with 'sk-proj-'" -ForegroundColor Red
            Write-Host "        Expected format: sk-proj-XXXX... (164 chars total)" -ForegroundColor Yellow
        }
        return $false
    }

    # Check length (164 characters)
    if ($ApiKey.Length -ne 164) {
        if (-not $Silent) {
            Write-Host "[ERROR] OpenAI API key must be exactly 164 characters (got $($ApiKey.Length))" -ForegroundColor Red
            Write-Host "        Expected format: sk-proj- + 156 alphanumeric/hyphen/underscore chars" -ForegroundColor Yellow
        }
        return $false
    }

    # Check character set (letters, numbers, hyphens, underscores)
    if ($ApiKey -notmatch '^sk-proj-[a-zA-Z0-9_-]{156}$') {
        if (-not $Silent) {
            Write-Host "[ERROR] OpenAI API key contains invalid characters" -ForegroundColor Red
            Write-Host "        Must be: sk-proj- followed by letters, numbers, hyphens, and underscores" -ForegroundColor Yellow
        }
        return $false
    }
    ...
    if (-not $Silent) {
        Write-Host "[OK] OpenAI API key format validated (164 chars, sk-proj-...)" -ForegroundColor Green
    }
    return $true
}
...
function Test-CohereApiKey {
    ...
    - Characters: Letters (a-z, A-Z) and numbers (0-9)
    - Example: N83CTHg45HoRRjH07S22GWGUVKPROsVEvQzJKsFL
    ...
    .EXAMPLE
    Test-CohereApiKey -ApiKey "N83CTHg45HoRRjH07S22GWGUVK..."
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($ApiKey)) {
        if (-not $Silent) {
            Write-Host "[ERROR] Cohere API key is empty" -ForegroundColor Red
        }
        return $false
    }

    # Check exact length (40 characters)
    ...
    if ($ApiKey -notmatch '^[a-zA-Z0-9]{40}$') {
        if (-not $Silent) {
            Write-Host "[ERROR] Cohere API key must contain only letters (a-z, A-Z) and numbers (0-9)" -ForegroundColor Red
            Write-Host "        Invalid characters detected in key" -ForegroundColor Yellow
        }
        return $false
    }
    ...
    # Check for placeholder patterns
    if (Test-PlaceholderPattern -Value $ApiKey -KeyType "Cohere" -Silent:$Silent) {
        return $false
    }

    if (-not $Silent) {
        Write-Host "[OK] Cohere API key format validated (40 chars, alphanumeric)" -ForegroundColor Green
    }
    return $true
}
